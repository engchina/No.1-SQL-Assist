    ...
    with gr.Tabs():
        with gr.TabItem(label="エージェント実行"):
            ...
            def _run_team(team_name, prompt):
                ...
                if not tn:
                    return gr.Markdown(visible=True, value="Team 名を入力してください"), gr.Textbox(visible=False, value="")
                if not q:
                    return gr.Markdown(visible=True, value="プロンプトを入力してください"), gr.Textbox(visible=False, value="")
                try:
